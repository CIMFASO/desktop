...
#include "ui_uistatsfactureslistview.h"
#include "utils/prixformat.h"
#include "utils/checkaccessutil.h"

UIStatsFacturesListView::UIStatsFacturesListView(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::UIStatsFacturesListView)
{
    ui->setupUi(this);
    ...
    connect(delegate,&CustomDelegate::btn1Clicked,this,&UIStatsFacturesListView::slotDetails);
    connect(delegate,&CustomDelegate::delButtonClicked,this,&UIStatsFacturesListView::slotDelete);
    connect(crud->getHttpObject(),&HttpRequest::requestSuccess,this,&UIStatsFacturesListView::httpResponse);
}

UIStatsFacturesListView::~UIStatsFacturesListView()
{
    delete ui;
}

void UIStatsFacturesListView::on_okPushButton_clicked()
{
...
void UIStatsFacturesListView::httpResponse(QMap<QString, QByteArray> response)
{
...
void UIStatsFacturesListView::slotDelete(const QModelIndex &index)
{
...
void UIStatsFacturesListView::slotDetails(const QModelIndex &index)
{
    crud->query("sel_details_facture.php?id="+QString::number(index.sibling(index.row(),0).data().toInt()));
}

void UIStatsFacturesListView::on_rechercherLineEdit_textChanged(const QString &arg1)
{
...
void UIStatsFacturesListView::on_tableView_clicked(const QModelIndex &index)
{
    slotDetails(index);
}

void UIStatsFacturesListView::on_attenteRadioButton_clicked()
{
...
void UIStatsFacturesListView::on_valideRadioButton_clicked()
{
    du = ui->duDateEdit->date().toString("yyyy-MM-dd");
    au = ui->auDateEdit->date().toString("yyyy-MM-dd");
...
void UIStatsFacturesListView::on_controleRadioButton_clicked()
{
    du = ui->duDateEdit->date().toString("yyyy-MM-dd");
    au = ui->auDateEdit->date().toString("yyyy-MM-dd");
...
void UIStatsFacturesListView::on_toutesRadioButton_clicked()
{
    du = ui->duDateEdit->date().toString("yyyy-MM-dd");
    au = ui->auDateEdit->date().toString("yyyy-MM-dd");
